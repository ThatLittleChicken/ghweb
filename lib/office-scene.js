// <office-scene> — low-poly home office background web component.
...
class OfficeScene extends HTMLElement {
  connectedCallback() {
    ...
    // build outside the hydration commit so the loader animation keeps a
    // frame between the two long tasks instead of one giant block
  ...
  applyTheme() {
    ...
    lampLight.intensity = dark ? 3.5 : 0; lampLight.distance = dark ? 2.6 : 3; lampLight.decay = dark ? 2.2 : 2; lampLight.color.set(dark ? '#ffb15c' : '#ffd58a'); bulb.emissiveIntensity = dark ? 1.0 : 0; bulb.emissive.set(dark ? '#ffb15c' : '#ffd58a');
  ...
  build() {
    ...
    // everything that casts shadows is static — render shadow maps once, not per frame
    ...
    const tick = (t) => {
      ...
      const a = 0.62 + (reduce ? 0 : Math.sin(t * 0.0002) * 0.14) + this.mouse.x * 0.22;
      camera.position.set(focus.x + Math.sin(a) * 6.6, 2.1 - this.mouse.y * 0.45 + (reduce ? 0 : Math.sin(t * 0.00034) * 0.08), focus.z + Math.cos(a) * 6.6);
    ...
    // compile shaders off the main thread where the driver supports it, so
    // the first rendered frame doesn't freeze the page for the compile
